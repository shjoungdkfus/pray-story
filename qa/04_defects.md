...
|---|---|
| ID | PS-AUTH-05 |
| 상태 | **🔶 조치 진행 중 (2026-08-04)** — 원인 A(OAuth 동의 화면 "테스트 중" + 테스트 사용자 0명) 확정. Google Cloud Console에서 로고 없음·승인된 도메인 없음·민감 scope 없음 확인 후 심사 제출 없이 **"프로덕션"으로 즉시 전환 완료**. 배포용(release) APK 재빌드·재설치 후 **소유자 본인 구글 계정 로그인은 정상 확인됨.** 다른 계정(아내)으로 재현 검증은 내일 예정 — 그 결과로 원인 A가 유일한 원인이었는지, 원인 후보 B(`_RootGate` 에러 처리)도 별도로 있는지 확정 |
...
| 근본 원인 | **후보 A(가능성 높음, 코드 밖 설정):** Google Cloud Console의 OAuth 동의 화면이 아직 "**테스트**" 게시 상태이면 콘솔에 등록된 **테스트 사용자 이메일 목록**에 없는 계정은 Google 인증 단계에서 자체적으로 거부된다. 이 경우 `GoogleSignIn.instance.authenticate()`(`login_screen.dart:31`)가 `GoogleSignInException`(코드는 `canceled`가 아님)을 던지고, `l.errGoogleFailed` 스낵바 후 `return`되어(`:42-45`) Supabase 세션 자체가 생기지 않는다 — 로그인 화면 복귀와 정확히 일치. **후보 B(코드 결함, 후보 A와 별개로도 존재):** 설령 세션이 정상 생성돼도, `_RootGate`(`main.dart:122-131`)는 `authStateProvider` 에러 시(`:124` `error: (e,_) => LoginScreen()`)와 `profileProvider` 에러 시(`:131` 동일 패턴) **원인 불문 무조건 LoginScreen으로 되돌린다.** 즉 프로필 조회 중 일시적 네트워크 문제·RLS 문제 등 어떤 예외가 나도 "로그인 안 된 것"과 구분 없이 로그인 화면으로 튕기고, **세션이 남아있어도 사용자에게는 원인이 전혀 보이지 않는다.** `profileProvider`(`profile_provider.dart:10-14`)의 `.maybeSingle()` 자체는 신규 유저(행 없음)를 정상적으로 `null`로 처리하므로 신규 프로필 부재가 원인은 아니고, **네트워크·RLS 등 다른 예외가 났을 때만 이 경로를 탄다** |
| 수정 제안 | (A) Google Cloud Console → API 및 서비스 → OAuth 동의 화면에서 게시 상태 확인. "테스트"면 어머니·아내 계정 이메일을 테스트 사용자로 추가하거나(즉시 해결), 요청 스코프가 profile/email뿐이면 검증 없이 "프로덕션"으로 게시 전환 가능한지 확인. (B, 코드 수정 필요) `_RootGate`의 두 `error` 분기를 무조건 `LoginScreen`으로 보내지 말고, 최소한 에러 로그를 남기고(현재 관찰용 로그도 없음) 재시도 버튼이 있는 에러 화면으로 분리해 "로그인 자격 없음"과 "일시적 오류"를 구분해야 함 |
| 회귀 위험 | (B) 수정 시 `_RootGate` 분기 로직 변경이라 로그인/온보딩/메인 전환 전체 경로에 영향 — 회귀 테스트 필요 |
| 검출 기법 | 오류 추정(사용자 실사용 보고) — 코드상 두 개의 독립된 실패 경로가 동일 증상(로그인 화면 복귀)을 만들 수 있음을 확인, 실기기 재현으로 어느 쪽인지(혹은 둘 다인지) 확정 필요 |
