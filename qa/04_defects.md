...
|---|---|
| ID | PS-UI-20 |
| 상태 | **🔴 2차 시도까지 실패 — 미해결 (2026-08-06).** 아래 "시도 기록" 참고. 원인 재조사부터 다시 시작해야 함 |
| 심각도 | S3 Major |
...
| **시도 기록 (2026-08-06)** | **1차**: `fontSize*2.2` 근사식을 `TextPainter(text: TextSpan(text:'가', style: style))..layout()`의 실측 `.height`로 교체(위 "수정 제안" 그대로), TextField `contentPadding`도 0으로 고정해 좌표계 일치 시도. → **실기기(SM S911N) 재현: 여전히 어긋남.** 사용자 스크린샷으로 직접 픽셀 측정한 결과, 실제 텍스트 줄 간격(≈59px)보다 계산된 pitch(≈82px)가 **약 1.39배 크게** 나와 아래로 갈수록 벌어짐 확인. **2차**: Flutter SDK 소스(`editable_text.dart:1120-1122`) 확인 결과 `TextField`가 `strutStyle` 미지정 시 내부적으로 `StrutStyle.fromTextStyle(style, forceStrutHeight: true)`를 자동 적용해 줄높이를 강제 통일한다는 것을 발견 — 이 strut 없이 잰 1차 계산이 실제보다 크게 나온 원인으로 추정하고, 페인터·TextField 양쪽에 동일한 explicit `StrutStyle`을 명시. → **실기기 재검증: 사용자가 "여전히 똑같아"라고 보고.** APK 재설치 타임스탬프(`firstInstallTime=lastUpdateTime=04:22:35`)로 재빌드 자체는 확인됐으므로 스테일 빌드 문제는 아님. **즉 strut 이론은 틀렸거나 불완전함 — 동일 스타일 텍스트에선 forceStrutHeight 유무가 실제로는 차이를 안 만들 가능성이 있어, 강제 균일 높이 자체는 원인이 아니었을 수 있다.** 다음 세션에서 원인 재조사부터 다시 시작할 것 — 직접 기기로 픽셀 단위 재측정(이번엔 내가 조사 도중 사용자가 세션 마무리를 요청해 중단됨, `test/onboarding_escape_test.dart`처럼 이 화면 전용 골든/스냅샷 검증도 고려) |
...
|---|---|
| ID | PS-UI-21 |
| 상태 | **✅ 수정 완료 + 실기기 검증 완료 (2026-08-06).** 사용자가 SM S911N에서 직접 확인 후 "커서 크기는 잘 맞는거 같다"고 확인 |
| 심각도 | S4 Minor |
| 관련 요구사항 | SPEC-GAP |
...
| 실제 수정 | 제안 그대로 `cursorHeight: fontSize * 1.2` 적용(`prayer_write_screen.dart`) |
| 회귀 위험 | 없음 — 시각적 조정만 |
...
|---|---|
| ID | PS-UI-22 |
| 상태 | **🟡 코드 수정 완료 — 실기기 검증 미완료 (2026-08-06).** 아래 "수정 내용" 참고. 사용자가 세션을 마무리해야 해서 화면 확인 전에 중단됨 — **다음 세션 최우선으로 검증할 것** |
| 심각도 | S3 Major |
| 관련 요구사항 | SPEC-GAP |
...
| 실제 수정 (2026-08-06) | 제안한 `\n`-split 방식은 **긴 줄이 자동 줄바꿈되면 소스 줄 인덱스와 렌더 줄(line metric) 인덱스가 안 맞을 수 있어** 그대로 쓰지 않음. 대신 `TextPainter.getLineBoundary(TextPosition)`으로 각 렌더 줄이 실제로 커버하는 **원본 문자열 범위**를 직접 조회해 `lineText.trim().isEmpty`로 판정하도록 교체 — 자동 줄바꿈 여부와 무관하게 항상 정확. `metric.width < 1.0` 조기 스킵은 제거(트림 검사가 더 포괄적이라 대체됨) |
| ⚠️ 미검증 | **실기기 확인 전 세션 중단.** 조사 중 다시 홈 화면을 스크린샷했을 때(`u1.png`) 마지막 텍스트 줄 아래로 짧은 밑줄 선이 하나 더 보이는 듯했으나 — 화질/각도상 확실히 판단하지 못했고 사용자 확인도 받지 못함. **다음 세션에서 가장 먼저 재확인할 것.** 정렬(어긋남) 쪽은 PS-UI-20의 strut 관련 원인이 여기에도 적용되는지 아직 조사 안 함 — `_UnderlinePainter`는 `computeLineMetrics()`를 쓰지 strut 관련 코드는 없어 이 화면엔 해당 안 될 가능성이 높지만 미확인 |
| 회귀 위험 | 낮음 |
...
|---|---|
| ID | PS-UI-23 |
| 상태 | **✅ 채택 + 수정 완료 + 실기기 검증 완료 (2026-08-06).** 성헌이 채택 승인, 아래 "수정 내용" 참고. SM S911N에서 칩·라벨 노출 확인됨("초대코드는 뜨고") |
| 심각도 | S4 Minor |
...
| 실제 수정 (2026-08-06) | `group_detail_screen.dart` 헤더 서브타이틀 줄 우측에 "초대코드 {code}" 칩 추가, 탭하면 클립보드 복사 + 기존 `inviteCodeCopied` 스낵바(기존 `InviteGroupScreen`의 복사 패턴 재사용). ARB 신규 `inviteCodeLabel`(ko "초대코드"/en "Invite code") — 처음엔 코드만 있어서 사용자 요청으로 라벨 추가 |
