...
| ID | 내용 | 근거 | 물어볼 것 |
|---|---|---|---|
| **SG-01** | 홈에서 인접 날짜로 이동하는 직접 수단이 없다(스와이프·날짜 피커 모두 부재). 기록탭 달력 경유만 가능 | `home_screen.dart` 전체에 `PageView`/`onHorizontalDrag`/피커 없음. git `c9ff357`에서 `PageView` 제거 | SPEC 4장 SCR-05는 이탈 경로를 "탭 이동, FAB→작성"으로만 적어 현재 코드와 모순 없음. **의도된 축소인가, 복구 대상인가?** |
| **SG-02** | 이메일 가입자용 **비밀번호 재설정 경로가 앱 전체에 없다** | `grep -rni "resetPassword\|forgot\|재설정" lib/` → 0건 | 비밀번호를 잊으면 계정 복구 불가. **출시 전 필수인가, 출시 후인가?** (Supabase는 `resetPasswordForEmail` 제공) |
| **SG-03** | 알람 삭제만 확인 다이얼로그가 없다(다른 삭제는 전부 있음) | `notification_settings_screen.dart:125-127` | 재생성이 쉬워 의도된 생략일 수 있음. **일관성을 맞출 것인가?** |
...
- **PS-A11Y-01**(아이콘 버튼 tooltip 전무)은 이번 스코프에서 제외 — 백로그 유지. 단 하단 탭·FAB에는 `Semantics`를 넣어 일부 개선됨.
