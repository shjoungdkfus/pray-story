...
| FR-016 | 이메일 가입자가 비밀번호를 잊었을 때 재설정할 수 있다 | P1 | SCR-PW | **확정(2026-07-28, SG-02 해소): 이메일 6자리 인증코드 방식.** Given 로그인 화면에서 "비밀번호를 잊으셨나요?" / When 이메일 입력 후 코드 요청 → 수신한 6자리 코드 + 새 비밀번호 입력 / Then `verifyOTP(type: recovery)` 성공 시 `updateUser`로 비밀번호 교체 후 signOut → 로그인 화면 복귀. 코드 소모 후 `updateUser` 실패 시 재요청 화면으로 되돌림(코드 재사용 불가하므로). 딥링크 대신 코드 방식을 쓴 이유: PKCE 딥링크는 요청 기기에서만 열어야 동작. **구현 완료(`password_reset_screen.dart`), 실기기 검증 미실시.** ⚠️**SMTP 미설정 상태에선 Supabase 기본 발송기(시간당 2통·팀 멤버 한정)라 실사용자에겐 메일이 안 감 — 출시 전 커스텀 SMTP 등록 필수.** |
...
### 8-2. 기도문 문서 상태 (2026-07-26 재확인 — B2 draft 구현 반영)

> ⚠️ 이 표는 최초 v0.1(2026-07-24 이전) 작성 당시 draft 자동저장(B2) 구현 전 서술이 남아있었음. 코드 재확인 결과 draft 기능이 이미 구현돼 있어 아래로 정정함(코드결함 아님, 문서 갱신).

| 현재 | 이벤트 | 다음 | 근거 |
|---|---|---|---|
| 신규 | 본문 입력 | 작성중(800ms 후 draft 자동저장, 신규모드만) | `write:107-118,128-149` |
| 작성중 | 저장 성공 | 저장됨(draft 폐기) | `write:209-217` |
| 작성중 | 저장 실패 | 작성중(스낵바, 입력 유지, draft 유지) | `write:233-238` |
| 작성중 | 이탈(X/취소, 신규모드) | **폐기되나 draft는 남아 다음 진입 시 복원**(`_restoreDraft`, 같은 날짜 한정) | `write:151-174` |
| 저장됨 | 수정 저장 | 저장됨 | `write:188-207` |
| 저장됨 | 삭제 | 삭제됨(hard, Undo 스낵바 5초 내 복구 가능) | `write:242-272`, FR-004 |
| 삭제됨 | 수정 시도(타기기 등에서 이미 삭제된 대상 update) | **불법 전이 — 2026-07-26 결함 확정 후 수정 완료.** 이전엔 0행 매칭돼도 에러 없이 "수정됨"으로 오인 표시됐음 → `.select()`로 실제 반영행 확인, 0건이면 `errPrayerNotFound` 스낵바로 안내(`write:188-207`) | `qa/04_defects.md` PS-CRUD-01 |

> **강제종료 후 재시작 복원:** 신규 작성 중(작성중) 상태는 800ms 디바운스로 draft 자동저장되어 **같은 날짜로 재진입 시 복원됨**(B2, FR-005). 수정 모드는 draft 대상 아님(원본이 서버에 남아있어 유실 없음, 편집 중이던 변경분만 유실 — PS-FLOW-03로 별도 기록됨).
...
## 11. 추적성 매트릭스 (4단계에서 케이스 도출 완료, 실행/결과는 5단계 이후)

| FR-ID | 화면 | 케이스 ID(대표) | 결과 |
|---|---|---|---|
| FR-001~003 | SCR-06/07 | `qa/03_testcases.md` TC-EQ-001~015, TC-ST-020~026 | 케이스 도출 완료, 실행 대기(일부는 기존 세션에서 실행됨) |
| FR-004/005 | SCR-06/07 | TC-ST-024, TC-ST-022 | **실기기 검증 완료(2026-07-24)** |
| FR-006 | 전역 | TC-ST-009, 4-2-1 #4 | 갭 확인만, 실행 미착수(재현 난이도 高) |
| FR-007/008 | SCR-12 | 07_rls_security_audit.md 참고 | 완료 |
| FR-009 | SCR-09 | TC-EQ-040~043, TC-ERR-12 | 케이스 도출 완료, **TC-EQ-042 결함후보 실행 권장** |
| FR-010 | SCR-10 | TC-PW-01~06 | 케이스 도출 완료, 실행 대기 |
| FR-011 | SCR-11 | 4-2-4, TC-ERR-10/13 | 케이스 도출 완료, 실행 대기 |
| FR-012 | 전역 | 13장 백로그 참고 | 실기기 검증 완료(알려진 제한 문서화됨) |
| FR-013 | 전역 | TC-PW-01~06 | 케이스 도출 완료, 실행 대기 |
| FR-014 | SCR-Rec | TC-ERR-05, TC-ERR-09 | 케이스 도출 완료, 실행 대기 |
| FR-015 | SCR-Com | 4-2-3, TC-EQ-050~053 | RLS는 완료(07 문서), 기능 P2는 케이스만 도출 |

> 상세 63개 개별 케이스 + 결정테이블은 `qa/03_testcases.md` 참고.
